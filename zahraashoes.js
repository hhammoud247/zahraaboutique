...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (itemContainer) {
        loadProductsIntoShoesSection(itemContainer);
    } else {
...
if (productForm) {
    productForm.addEventListener('submit', function (event) {
        ...
        saveProductToLocalStorage(product_shoes);
...
function saveProductToLocalStorage(product_shoes) {
    const existingProducts = JSON.parse(localStorage.getItem('products1')) || [];
    existingProducts.push(product_shoes);
    localStorage.setItem('products1', JSON.stringify(existingProducts));
}

// Load products into the clothes section (only for the Clothes page)
function loadProductsIntoShoesSection(itemContainer) {
    const products = JSON.parse(localStorage.getItem('products1')) || [];
    products.forEach(product_shoes => {
        const itemCard = createItemCard(product_shoes);
        itemContainer.appendChild(itemCard);
    });
}

// Create the HTML for each product card
function createItemCard(product_shoes) {
    const itemCard = document.createElement('div');
    itemCard.classList.add('product-container');
    itemCard.innerHTML = `
        <img class="product-image" src="${product_shoes.image}" alt="${product_shoes.title}">
        <div class="product-title-input">
            <input type="text" class="product-title-input" value="${product_shoes.title}" readonly />
        </div>
        <div class="product-title-input">
            <span>Price:</span>
            <input type="text" class="product-title-input" value="$${product_shoes.price}" readonly />
        </div>
